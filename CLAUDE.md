...
poe pre-commit   # Run pre-commit hooks on all files
...
- **Batched 1Password resolution:** `_resolve_secrets` takes a dict of env vars and resolves every `op://` ref in a single `op inject` subprocess call. The template is `.env`-style (`KEY={{ op://... }}` per line) and parsing splits on the first `=` so values containing `=` stay intact. Values must be single-line (fine for SSO creds and Base32 TOTP keys). Non-zero exits retry with linear backoff, stderr logged each time. Called from a `secrets-prefetch` daemon thread started at the top of `download_docs()` — before any worker thread or browser launches — so the 1Password prompt appears immediately. The `op://` refs are removed from `os.environ` (`os.environ.pop`) as soon as resolution succeeds. `_login()` then waits on the event and reads credentials from the shared result dict instead of touching the environment.
